...
var subarraySum = function(nums, k) {
  ...
  // set 0 1是说明总有几个数的和与k相等，这时候我们总得设置一个出口就是0，其余的都不行
  // ps: J ＝ i - k第一次相等是从0累是的，后面可能有新值，导致i -k等的这个值正好为我们前面计算的和
  // [3,4,7,2,-3,1,4,2]
  // mp: {(0,1),(3,1),(7,1),(14,1)...}
  // 第一次3 4为7 7减去7为0，第二次14减7为7 7正好我们算过，所以正好7就是前面算的新值
  map.set(0, 1);
  // sum_i [0..i]的和
  var res = 0, sum0_i = 0;
  for(var i = 0; i < n; i++) {
